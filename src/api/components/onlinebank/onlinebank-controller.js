...
async function create(request, response, next) {
  ...
  try {
    var bankAccount = await onlineBankService.createBankAccount(
      userId,
      accountNumber
    );
...
async function topUp(request, response, next) {
  ...
  try {
    var bankAccount = await onlineBankService.topUp(
      accountNumber,
      userId,
      total
    );
...
async function getBankAccountByAccountNumber(request, response, next) {
  try {
    const token = request.headers.authorization;
    const userId = decodeToken(token);
    const accountNumber = request.params.account_number; // nomer rekening
