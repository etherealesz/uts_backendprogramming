...
async function withdraw(accountNumber, userId, total) {
  try {
      ...
      let responseTransactions = await bankAccountRepository.withdraw(bankAccounts.accountNumber, total);
...
async function topUp(accountNumber, userId, amount) {
  ...
  return bankAccountRepository.topUp(bankAccount.id, amount);
}
